...
\todo{Ask semonti for the figure}
\todo{Zoom in on the middle with an adapted scalebar, also make notations in the image with the expected properties (mono-bilayer etc)}
\todo{Make one figure of the the 2 columns (a and b), with one caption}

\todo{monash sewtup ref to lab Michael Fuhrer}
\begin{figure}[H]
  \centering
...
\todo{Find a way to put this in the results (choice was made before designing but still part of the process)}
...
\todo{Add the vibration isolation part here because it is important for the complete design cycle}

\todo{Also explain the subdivisions of the project and where to find what content}
...
\todo{refer back to the complete image for a visualisation of the mask omponents}
...
\todo{Create a new render of the original model}
...
\todo{Remove the grid from all te graphs}
