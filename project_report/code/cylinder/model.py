...
class GridConditions:
    ...
    if state not in ["SOLID", "REDUCED"]:
        raise TypeError(
            "The model type should either be SOLID or REDUCED, not {}".format(state)
        )
    ...
    # t_list = [round(i/2, 2) for i in range(2 * 10 * 60)]
    ...
    def D(self) -> float:
        ...
        if (
            self.position < self.switch_points[0]
            or self.position > self.switch_points[1]
        ):
            return self.Dn
        else:
            return self.Dal
...
def run_model(conditions):
    ...
    print(
        "{} timesteps, {} gridpoints = {} calculations".format(
            par.t_list[-1] / par.h, par.N, par.t_list[-1] / par.h * par.N
        )
    )
